...
class WaColor {
  ...
  // 表示に使用するFlutterのColorを返す。
  Color toColor() {
    if (color == null) {
      final rgb = RGB.fromHex(colorCode);
  ...
  String hslString() {
    final hsl = HSL.fromHex(colorCode);
...
class ColorManager {
  ...
  ColorManager._() {
    ...
    for (final line in colorCsv.split('\n')) {
      ...
      final v = line.split(',');
      colorList.add(WaColor(v[0], v[1], v[2]));
    }
  }
  ...
  // 引数で渡された名前を持つ色を返す。マッチするものがなかったらnullを返す。
  WaColor? colorNamed(String s) {
    for (final v in colorList) {
      if (v.isEqualName(s)) {
        return v;
      }
    }
